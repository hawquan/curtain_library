    ...
    this.actroute.queryParams.subscribe(a => {
      ...
      this.http.post('https://curtain.vsnap.my/getonesales', { no: this.sales_id }).subscribe((s) => {
        ...
        this.http.post('https://curtain.vsnap.my/getthismonthsales2', { month: this.datepipe.transform(new Date(), 'MMyyyy') }).subscribe(a => {
          ...
          if (this.thismonthsales.length > 0 && this.info.so_pdf.length > 0) {
            let regex = /^(\d{1,2})_/;
            let match = this.info.so_pdf[this.info.so_pdf.length - 1].name.match(regex);
            let digits = match[1];

            let startIndex = digits.length + 1; // Start index for the substring
            let endIndex = startIndex + 4; // End index for the substring (exclusive)

            let extractedString1 = this.info.so_pdf[this.info.so_pdf.length - 1].name.slice(startIndex, endIndex);

            if (extractedString1 == this.datepipe.transform(new Date(), 'yyMM')) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let temp = {
          no: this.sales_id,
          step: 2,
          nf_remark: `Quotation generated by '` + this.user['name'],
          uid_by: this.user['id'],
          uid_to: null,
          notif: true
        }
        ...
        if (this.item[i].motorized_upgrade && (this.item[i].motorized_choice == 'Both' || this.item[i].motorized_choice == 'Sheer')) {
          ...
          if (this.item[i].track_sheer) {
            if (this.item[i].track_sheer == 'Ripplefold' || this.item[i].track_sheer.includes('Existing Ripplefold')) {
              ...
              if (this.item[i].fabric_type == 'CS' && this.item[i].sheer_bracket != 'Ceiling Pelmet') {
                S_height = height - 1.25
              } else {
                S_height = height - (this.item[i].sheer_bracket == 'Ceiling Pelmet' ? 1.25 : 0.25)
              }
      ...
      if (this.item[i].type != 'Blinds') {
        ...
        if (this.item[i].pleat == 'Roman Blind') {
          ...
          if (this.item[i].fabric != null) {
            let fabricWidth
            if (this.fabricCurtain.filter(x => x.name == this.item[i].fabric)[0]) {
              fabricWidth = this.fabricCurtain.filter(x => x.name == this.item[i].fabric)[0]['size']
            } else {
              fabricWidth = this.fabricSheer.filter(x => x.name == this.item[i].fabric)[0]['size']
            }
        ...
        this.http.post('https://forcar.vsnap.my/uploadPDF', { base64: data }).subscribe((link) => {
          ...
          if (this.info.sales_confirmed_date != null) {
            temp = {
              ...
              nf_remark: `Quotation & Sales Order generated by '` + this.user['name'],
              uid_by: this.user['id'],
              uid_to: null,
              notif: true
            }
