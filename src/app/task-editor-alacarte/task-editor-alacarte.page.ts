...
@Component({
  selector: 'app-task-editor-alacarte',
  templateUrl: './task-editor-alacarte.page.html',
  styleUrls: ['./task-editor-alacarte.page.scss'],
})

export class TaskEditorAlacartePage implements OnInit {
  ...
  back() {
    ...
    if (this.item.track || this.item.fabric_type || this.item.bracket || this.item.width || this.item.location) {
      Swal.fire({
        title: 'Close Update',
        text: 'Your order will not be updated. Are you sure?',
        heightAuto: false,
        icon: 'warning',
        showConfirmButton: true,
        showCancelButton: true,
